...
export default function ConnectAccountsGrid({ accounts = [], onConnectPlatform }) {
  ...
  for (let i = 0; i < PLATFORM_LIST.length; i++) {
    ...
    if (connectedPlatformIds.includes(platform.id.toLowerCase())) {
      isConnected = true;
    }

    let cardBorderClass = 'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800 hover:border-slate-300 dark:hover:border-slate-700 hover:shadow-sm';
    if (isConnected) {
      cardBorderClass = 'border-emerald-200 dark:border-emerald-900/60 bg-emerald-50/30 dark:bg-emerald-950/30';
    }
    ...
    let scopeText = 'Connect and publish';
    if (platform.scopes) {
      if (platform.scopes.length > 0) {
        if (platform.scopes[0]) {
          scopeText = platform.scopes[0];
        }
      }
    }
    ...
    platformCards.push(
      <div
        key={platform.id}
        className={`p-4 rounded-2xl border transition-all flex flex-col justify-between ${cardBorderClass}`}
      >
  ...
  if (totalPlatforms > 0) {
    progressPercent = Math.round((connectedCount / totalPlatforms) * 100);
  }
